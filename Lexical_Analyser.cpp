#include<iostream>
#include<fstream>
std::string keywords[20] = {"char","break","const","continue","double","default","else","float","int","str",
						"if","long","case","func","for","until","write","read","readLine","return"};
...
int IsKeyWord(std::string char_stream,int forward)
{
	...
	if(Match_Keyword(word) && char_stream[forward] != '_') return forward;
	else return -1;
}
